...
@click.option(
    ...
    default=os.path.join(
        PEREIRA_OUTPUT_DIR, "rsa_correlations_spearman_cosine_paragraphs.csv"
    ),
)
@click.option(
    ...
    type=str,
    default=os.path.join(
        PEREIRA_OUTPUT_DIR, "pereira_neural_enc_passages_pairwise_accuracy.csv"
    ),
)
...
def plot_corr_rsa_neural_enc(
    rsa_input_file: str = os.path.join(
        PEREIRA_OUTPUT_DIR, "rsa_correlations_spearman_cosine_paragraphs.csv"
    ),
    ...
    rsa_corr = rsa_corr.melt(
        id_vars=["index"],
        value_vars=[col for col in rsa_corr.columns if "index" not in col],
    )
    rsa_corr = rsa_corr.rename(columns={"variable": "roi", "value": "rsa_corr"})
    neural_enc = neural_enc.melt(
        id_vars=["index"],
        value_vars=[col for col in neural_enc.columns if "index" not in col],
    )
...
@click.option(
    "--sent_eval_mode", type=click.Choice(["sts", "transfer"]), default="transfer"
)
@click.option("--per-brain-network", type=bool, default=False)
...
def plot_corr_rsa_sent_eval(
    ...
    # exist)
    rsa_corr_df = pd.read_csv(
        rsa_input_file, index_col=0, usecols=lambda x: "p-value" not in x
    )
    for model_name in rsa_corr_df.index:
        try:
            file_path = glob(
                os.path.join(
                    sent_eval_results_dir,
                    model_name.split("/")[-1] + f"_{sent_eval_mode}_test.csv",
                ),
            )[0]
            rsa_corr_df.loc[model_name, "sent_eval_avg"] = pd.read_csv(file_path)[
                "Avg."
            ][0]
        except IndexError:
            continue
...
def plot_scatter_regplot(
    ...
    color: str = "#CF995F",
    output_path: str = os.path.join(PEREIRA_OUTPUT_DIR, "corr_rsa_neural_enc.png"),  # noqa
    ...
    """Plot line/regression plots between various variables from various analyses.
    ...
    if per_brain_network:
        for i, b in enumerate(brain_networks):
            ...
            # Calcualte the correlation
            corr, p_value = CORRELATION_MEASURES["pearson"](
                network_specific_x, network_specific_y
            )
...
@click.command()
@click.option(
    "--paragraph-input-path", type=str, default=PEREIRA_PERMUTED_SENTENCES_PATH
)
...
def plot_correlogram_sent_models(
    ...
    for sent_model_name in sent_model_names:
        ...
        sent_model_rdms[sent_model_name] = get_sim_vector(
            model.generate_rdm(paragraphs)
        )
        del model

    # 2. Get all pairwise correlations
    pair_corr_df = pd.DataFrame.from_dict(sent_model_rdms).corr(method="pearson")
    pair_corr_df = pair_corr_df.rename(
        columns=SENT_EMBED_MODEL_NAMES_EN, index=SENT_EMBED_MODEL_NAMES_EN
    )
...
@click.option(
    ...
    type=str,
    default=os.path.join(
        PEREIRA_OUTPUT_DIR, "pereira_neural_enc_passages_pairwise_accuracy_ridge.csv"
    ),
)
@click.option("--color", type=str, default="#CF995F")
@click.option("--output-dir", type=str, default=PEREIRA_OUTPUT_DIR)
def plot_corr_neural_enc_embed_size(
    neural_enc_input_file: str = os.path.join(  # noqa
        PEREIRA_OUTPUT_DIR,
    ...
    neural_enc_df = pd.read_csv(
        neural_enc_input_file, index_col=0, usecols=lambda x: "p-value" not in x
    )
    ...
    neural_enc_df = neural_enc_df.rename(
        columns={"variable": "roi", "value": "neural_enc"}
    )
...
@click.group()
def cli() -> None:
    """Generate a correlation plot between RSA correlations and neural encoding scores."""
